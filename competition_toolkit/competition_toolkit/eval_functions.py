...
# General util function to get the boundary of a binary masks.
def _mask_to_boundary(mask, dilation_ratio=0.02):
    ...
    if dilation < 1:
        dilation = 1
    # Pad image so masks truncated by the image border is also considered as boundary.
...
def biou(dt, gt, dilation_ratio=0.02):
    ...
    :param gt (numpy array, uint8): binary masks
    :param dt (numpy array, uint8): binary masks
    :param dilation_ratio (float): ratio to calculate dilation = dilation_ratio * image_diagonal
